...
# } 
set i 0 
set mnode_(0) [$ns_ node] 
incr i  
set mnode_(1) [$ns_ node] 
incr i  
set mnode_(2) [$ns_ node] 
incr i  
set mnode_(3) [$ns_ node] 
incr i  
set mnode_(4) [$ns_ node] 
incr i  
set mnode_(5) [$ns_ node] 
incr i  
set mnode_(6) [$ns_ node] 
incr i  
set mnode_(7) [$ns_ node] 
incr i  
set mnode_(8) [$ns_ node] 
incr i  
set mnode_(9) [$ns_ node] 
incr i  
...
$ns_ attach-agent $mnode_(0) $sink_(0)  
 
set udp_(0) [new Agent/UDP] 
$mnode_(6) setdest [$mnode_($d($i)) set X_] [$mnode_($d($i)) set Y_] 0  
$ns_ attach-agent $mnode_(6) $udp_(0)  
...
$cbr_(0) set packet_size_ 50 
$cbr_(0) set rate_ 0.1Mb 
$cbr_(0) set interval_ $opt(interval) 
#$cbr set random_ false  
...
$ns_ attach-agent $mnode_(9) $sink_(1)  
 
set udp_(1) [new Agent/UDP] 
$mnode_(2) setdest [$mnode_($d($i)) set X_] [$mnode_($d($i)) set Y_] 0  
$ns_ attach-agent $mnode_(2) $udp_(1)  
...
$cbr_(1) set packet_size_ 50 
$cbr_(1) set rate_ 0.1Mb 
$cbr_(1) set interval_ $opt(interval) 
#$cbr set random_ false  
 
$ns_ at [expr 100 + [expr $i - 1] * $opt(interval) / $opt(tn)] "$cbr_($i) start"  
$ns_ at [expr $opt(stop) - 5] "$cbr_($i) stop"  
incr i  
set sink_(2) [new Agent/Null] 
$ns_ attach-agent $mnode_(7) $sink_(2)  
 
set udp_(2) [new Agent/UDP] 
$mnode_(9) setdest [$mnode_($d($i)) set X_] [$mnode_($d($i)) set Y_] 0  
$ns_ attach-agent $mnode_(9) $udp_(2)  
...
$cbr_(2) set type_ CBR 
$cbr_(2) set packet_size_ 50 
$cbr_(2) set rate_ 0.1Mb 
$cbr_(2) set interval_ $opt(interval) 
#$cbr set random_ false  
